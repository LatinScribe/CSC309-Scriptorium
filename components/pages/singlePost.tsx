  ...
  const handleReplySubmit = async (parentCommentId: number, reply: string) => {
    ...
    try {
      if (session) {
        ...
        const addReplyToComments = (commentsList: Comment[], parentId: number, reply: Comment): Comment[] => {
          return commentsList.map((comment) => {
            if (comment.id === parentId) {
              return {
                ...comment,
                replies: [...(comment.replies || []), { ...reply, author: session.user }]
              };
        ...
        setComments((prev) => addReplyToComments(prev, parentCommentId, replyResponse));
        console.log(comments);
