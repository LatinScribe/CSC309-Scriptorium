...
import { Comment as OriginalComment } from "@/utils/backendTypes";

interface Comment extends Omit<OriginalComment, 'author'> {
    author: {
        username: string;
    };
}
    ...
    if (req.method === 'GET') {
        try {
            ...
            if (sortOption === 'mostUpvotes') {
                ...
                orderBy = [
                    { createdAt: 'desc' as const }, // default sort by creation time
                    { id: 'asc' as const }
                ];
            }
            ...
            const mapCommentsToParent = (comments: Comment[]) => {
            ...
            res.status(500).json({ error: "Could not fetch comment", details: (error as Error).message });
        }
        ...
        const { content, parentCommentId, blogPostId } = req.body; // extract from req body
